...
def extract_near_surface_volume_fn(input_tensor: torch.Tensor, alpha: float):
    """
    PyTorch implementation with dimension fix.
    Args:
    ...
    valid_mask = val > -9000  # assume -9000 is invalid

    # Improved neighbor retrieval function (maintains dimension consistency)
    def get_neighbor(t, shift, axis):
        """Shift along specified axis while maintaining dimension consistency"""
        if shift == 0:
            return t.clone()
        ...
        # Execute padding (add batch and channel dims to match F.pad)
        padded = F.pad(
            t.unsqueeze(0).unsqueeze(0), pad_dims[::-1], mode="replicate"
        )  # reverse order to match F.pad
    ...
    # Handle boundary invalid values (use where to maintain dimension consistency)
    ...
    # Compute sign consistency (convert to float32 for precision)
...
def pymeshlab2trimesh(mesh: pymeshlab.MeshSet):
    ...
    # Check the loaded object type
    if isinstance(mesh, trimesh.Scene):
        combined_mesh = trimesh.Trimesh()
        # If Scene, iterate all geometry and merge
